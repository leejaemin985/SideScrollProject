...
{
    public Color defaultColor { get; set; } = new Color(0f, 1f, 1f);

    public Color detectedColor { get; set; } = new Color(1f, 0f, 0f);
...
public static class PhysicsExt
{
        ...
        )
    {
        ...
        {
            start = ray.origin,
            end = (ret ? hitInfo.point : ray.origin + ray.direction * maxDistance),
            isHit = ret
        };
    ...
    public static void DrawGizmoBox(in DrawGizmoCubeInfo info)
    {
        ...
        Gizmos.DrawWireCube(info.start, info.halfSize * 2f);


        Gizmos.color = info.isHit ? info.detectedColor : info.defaultColor;
        Gizmos.DrawLine(info.start, info.end);
        Gizmos.DrawWireCube(info.end, info.halfSize * 2);
    }
